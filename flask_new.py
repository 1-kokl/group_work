...
def user_register():
    ...
    if not re.match(r"^[a-zA-Z0-9_\u4e00-\u9fa5]+$", username):
        return jsonify({"code": 400, "msg": "用户名仅支持字母、数字、下划线、中文"}), 400
...
def add_product():
    ...
    if not name or price is None:
        return jsonify({"code": 400, "msg": "商品名称和价格不能为空"}), 400

    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "INSERT INTO products (name, price, description, seller_username) VALUES (?, ?, ?, ?)",
            (name, float(price), description, current_username)
        )
        conn.commit()
        return jsonify({"code": 200, "msg": "添加成功", "data": {"name": name, "price": price}}), 201
    except Exception as e:
        conn.rollback()
        return jsonify({"code": 500, "msg": f"服务器错误: {str(e)}"}), 500
    finally:
...
def get_my_products():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM products WHERE seller_username = ? ORDER BY created_at DESC", (current_username,))
        rows = cursor.fetchall()
        my_products = [dict(row) for row in rows]
        return jsonify({"code": 200, "data": my_products, "msg": "success"})
    except Exception as e:
        return jsonify({"code": 500, "msg": f"服务器错误: {str(e)}"}), 500
    finally:
        cursor.close()
