  ...
  username: [
    { required: true, message: '请输入用户名', trigger: 'blur' },
    {
      validator: async (_, value, callback) => {
        ...
        // 修复：调用checkUsername返回对象，判断valid字段
        const result = checkUsername(sanitized);
        if (!result.valid) {
          callback(new Error(result.feedback[0]));
          return;
        }
  ...
  phone: [
    { required: true, message: '请输入手机号', trigger: 'blur' },
    {
      validator: (_, value, callback) => {
        ...
        // 修复：调用checkPhone返回对象，判断valid字段
        const result = checkPhone(sanitized);
        if (!result.valid) {
          callback(new Error(result.feedback[0]));
        } else {
          callback();
...
// 其余逻辑（watch/handleSubmit/goLogin）保持不变
watch(
  () => form.password,
  () => {
    if (formRef.value) {
...
async function handleSubmit() {
  ...
  formRef.value.validate(async (valid) => {
    ...
    loading.value = true;
    try {
      // 可选：如需加密密码/手机号，启用以下代码
      // const hashedPwd = hashPassword(form.password);
      // const encryptedPhone = encryptPhone(form.phone);

      await store.dispatch('auth/register', {
        username: sanitizeInput(form.username),
        email: sanitizeInput(form.email),
        password: form.password, // 替换为hashedPwd（如需加密）
        phone: sanitizeInput(form.phone) // 替换为encryptedPhone（如需加密）
      });

      await store.dispatch('auth/login', {
        identifier: sanitizeInput(form.username),
        password: form.password, // 注意：登录时需和后端加密逻辑一致
        remember: false
      });
