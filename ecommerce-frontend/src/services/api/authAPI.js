...
function normalizeAuthResponse(data = {}, meta = {}) {
  ...
  const refreshToken =
    ...
    null;
  const expiresIn =
    p.expiresIn || p.expires_in || data.expiresIn || data.expires_in ||
    data?.data?.expiresIn || data?.data?.expires_in || null;
  const csrfToken =
    p.csrfToken || p.csrf_token || data.csrfToken || data?.data?.csrfToken || null;
  const inferredUser =
    p.user ||
    data.user ||
    data?.data?.user ||
    ...
    data?.data?.email
      ? {
          username:
            meta.username || p.username || data.username || data?.data?.username || null,
          role: meta.role || p.role || data.role || data?.data?.role || 'user',
          roles:
            meta.roles ||
            p.roles ||
            [meta.role || p.role || data.role || 'user'].filter(Boolean),
          phone: meta.phone || p.phone || data.phone || data?.data?.phone || null,
          email: meta.email || p.email || data.email || data?.data?.email || null
        }
      : null);
